...
public class Mastermind {
    ...
    public boolean nouvellePartie(Joueur joueur) {
        Optional optional = partieRepository.getPartieEnregistree(joueur);
        if(optional.isEmpty()) {
    ...
    public ResultatPartie evaluation(Joueur joueur, String motPropose) {
        ...
        Partie partie = (Partie) optional.get();
    ...
    private boolean isJeuEnCours(Optional<Partie> partieEnCours) {
        return partieRepository.getPartieEnregistree(partieEnCours.get().getJoueur()).isPresent();
    }
}
